...
transfer_notice_db = DB["trademark"]["revoke"]


def ocr_img(img_path,imgName):
    img_dict = {
        "first_img": [r"E:\img\revoke\ocr\first_img.jpg", (0, 0, 1110, 135)],
        "second_img": [r"E:\img\revoke\ocr\second_img.jpg", (101, 137, 1083, 370)],
        "three_img": [r"E:\img\revoke\ocr\three_img.jpg", (103, 370, 1103, 1530)],
    ...
    for _ in img_lt:
        ...
        if count == 1:
            ...
            res_dict["document_number"] = result[3].split(":")[1].strip("/尽")
            res_dict["reasons_for_revocation"] = result[4].split(":")[1].strip("ˇ齐/7.$人")
            res_dict["ServiceItems"] = result[5].split("服务项目")[1].strip(">Z2")
...
import os
file_lt = os.listdir(r'E:\img\revoke')

for _ in file_lt:
    if _ == "ocr":
        continue
    else:
        img_path = r"E:\img\revoke\{}".format(_)
        print(img_path)
        imgName = _.split(".")[0]
