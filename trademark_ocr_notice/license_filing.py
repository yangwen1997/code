...
transfer_notice_db = DB["trademark"]["license_filing"]


def ocr_img(img_path,imgName):
    img_dict = {
        "first_img": [r"E:\img\license_filing\ocr\first_img.jpg", (0, 0, 1110, 135)],
        "second_img": [r"E:\img\license_filing\ocr\second_img.jpg", (101, 137, 1083, 227)],
        "three_img": [r"E:\img\license_filing\ocr\three_img.jpg", (103, 227, 1103, 780)],
    }
    ...
    for _ in img_lt:
        ...
        if count == 1:
            ...
            # 获取图片的公告类别
            res_dict["notice_category"] = result[0].strip("'/8y4")

        elif count == 3:
            # 块，注册号 ,id ,商标，类别，备案号,许可使用的期限，许可人，被许可人，许可使用的商品/服务项目
            res_dict["block"] = "1"
            res_dict["registration_number"] = result[0].split(":")[1].strip("公")
            ...
            res_dict["record_no"] = result[3].split(":")[1]

            res_dict["service_life"] = result[4].split(":")[1]
            res_dict["licensors"] = result[5].split(":")[1]
            res_dict["licensees"] = result[6].split(":")[1].strip("人")
            res_dict["service_items"] = result[7].split(":")[1].strip("y7l")
...
import os
file_lt = os.listdir(r'E:\img\license_filing')

for _ in file_lt:
    if _ == "ocr":
        continue
    else:
        img_path = r"E:\img\license_filing\{}".format(_)
        print(img_path)
        imgName = _.split(".")[0]
