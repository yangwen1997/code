...
transfer_notice_db = DB["trademark"]["invalid"]


def ocr_img(img_path,imgName):
    img_dict = {
        "first_img": [r"E:\img\invalid\ocr\first_img.jpg", (0, 0, 1110, 135)],
        "second_img": [r"E:\img\invalid\ocr\second_img.jpg", (101, 137, 1083, 384)],
        "three_img": [r"E:\img\invalid\ocr\three_img.jpg", (103, 384, 1103, 1530)],
    ...
    for _ in img_lt:
        ...
        if count == 1:
            ...
            # 获取图片的公告类别
            res_dict["notice_category"] = result[0].strip("*7")
            ...
            res_dict["document_number"] = result[3].split(":")[1].strip("/这")
            res_dict["reason"] = result[4].split(":")[1].strip("ˇ齐/7.$人")
...
import os
file_lt = os.listdir(r'E:\img\invalid')

for _ in file_lt:
    if _ == "ocr":
        continue
    else:
        img_path = r"E:\img\invalid\{}".format(_)
        print(img_path)
        imgName = _.split(".")[0]
