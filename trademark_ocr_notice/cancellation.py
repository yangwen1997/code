...
transfer_notice_db = DB["trademark"]["cancellation"]


def ocr_img(img_path,imgName):
    img_dict = {
        "first_img": [r"E:\img\cancellation\ocr\first_img.jpg", (0, 0, 1110, 135)],
        "second_img": [r"E:\img\cancellation\ocr\second_img.jpg", (101, 137, 1083, 420)],
        "three_img": [r"E:\img\cancellation\ocr\three_img.jpg", (103, 420, 1103, 1530)],
    ...
    for _ in img_lt:
        ...
        if count == 1:
            ...
            # 获取图片的公告类别
            res_dict["notice_category"] = result[0].strip("47足ˇ")
            ...
            res_dict["logout_date"] = result[3].split(":")[1].strip("`7'人")

            if "服务项目" in result[5]:
                res_dict["registrant"] = result[4].split("注册人")[1].strip("/一K 人")
                res_dict["ServiceItems"] = result[5].split(":")[1].strip("<")
            else:
                res_dict["registrant"] = result[4].split("注册人")[1].strip("/一K 人") + result[5]
                res_dict["ServiceItems"] = result[6].split(":")[1].strip("<")
...
import os
file_lt = os.listdir(r'E:\img\cancellation')

for _ in file_lt:
    if _ == "ocr":
        continue
    else:
        img_path = r"E:\img\cancellation\{}".format(_)
        print(img_path)
        imgName = _.split(".")[0]
