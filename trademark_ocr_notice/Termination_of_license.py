...
transfer_notice_db = DB["trademark"]["Termination_of_license"]


def ocr_img(img_path,imgName):
    img_dict = {
        "first_img": [r"E:\img\Termination_of_license\ocr\first_img.jpg", (0, 0, 1110, 135)],
        "second_img": [r"E:\img\Termination_of_license\ocr\second_img.jpg", (101, 137, 1083, 210)],
        "three_img": [r"E:\img\Termination_of_license\ocr\three_img.jpg", (103, 210, 1103, 645)],
        # "four_img": [r"E:\img\Termination_of_license\ocr\four_img.jpg", (103, 516, 1103, 825)],
        # "five_img": [r"E:\img\Termination_of_license\ocr\five_img.jpg", (103, 825, 1103, 1135)],
        # "six_img": [r"E:\img\Termination_of_license\ocr\six_img.jpg", (103, 1135, 1103, 1443)],
    }
    ...
    for _ in img_lt:
        ...
        if count == 1:
            ...
            # 块，注册号
            res_dict["block"] = "1"
            res_dict["registration_number"] = result[0].split(":")[1].strip("′")[:-2]
            _id = prefix + f"_{res_dict['registration_number'].strip('`')}"
            res_dict["_id"] = hashlib.md5(_id.encode('utf-8')).hexdigest()
            res_dict["trade"] = result[1].split(":")[1].strip("为念")
            res_dict["category"] = result[2].split(":")[1].strip("尽")

            res_dict["License_filing"] = result[3].split(":")[1].strip("人/s&")

            if "被许可人" in result[5]:
                res_dict["License_person"] = result[4].split(":")[1]
                res_dict["Permission_filing"] = result[5].split(":")[1]
                res_dict["Termination_time"] = result[6].split(":")[1]
            else:

                res_dict["License_person"] = result[4].split(":")[1] + result[5]
                res_dict["Permission_filing"] = result[6].split(":")[1]

                res_dict["Termination_time"] = result[7].split(":")[1]
...
import os
file_lt = os.listdir(r'E:\img\Termination_of_license')

for _ in file_lt:
    if _ == "ocr":
        continue
    else:
        img_path = r"E:\img\Termination_of_license\{}".format(_)
        print(img_path)
        imgName = _.split(".")[0]
