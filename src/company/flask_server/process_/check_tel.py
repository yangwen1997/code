...
class CheckTel(object):
    ...
    def check_after(self):
        ...
        # 1. 读取文本数据统一添加到列表中
        realPath = r'D:\bmd\bmd_server\src\company\flask_server\files\{}\TEL\活跃号(实号).txt'.format(self.userid)
    ...
    def down_api(self,sendIDS):
        ...
        try:
            ...
            # zipNameLt = zfile.namelist()
            outPath = file_paths = r'D:\bmd\bmd_server\src\company\flask_server\files\{}\TEL'.format(self.userid)
            # for _ in zfile.namelist():
            #     try:
            #         _ = _.encode('cp437').decode('gbk')
            #         # path = outPath + r"\{}".format(_)
            #         zfile.extract(_,outPath)
            #     except:
            #         _ = _.encode('utf-8').decode('utf-8')
            #         zfile.extract(_, outPath)
            zfile.extractall(path=outPath)
            zfile.close()
            os.remove(file_path)
            # 这里需要修改zipfile模块中的源码的编码cp437为gbk
