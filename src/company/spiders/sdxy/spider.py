...
import threading
import hashlib
sys.path.insert(0,r"D:\bmd\bmd_server\spiders")
from spiders.common import get_log,ABY
import requests
from lxml.etree import HTML
from sdxy.XPATH import WaterCredit
...
class Spider(object):
    ...
    def base_first_page(self, items, resp,url):
        ...
        base["holderInfoCount"] =  None if holderInfoCount == '' else holderInfoCount
        gdxx = self.gdxx(resp)
        base["holderInfo"] = gdxx

        # 主要人员计数 主要人员
        employeeInfoCount = "".join(etre.xpath('//span[contains(text(),"主要成员")]/following-sibling::span[1]/text()'))
        base["employeeInfoCount"] = None if employeeInfoCount == '' else employeeInfoCount
        ...
        base["changeInfoCount"] = None if changeInfoCount == '' else changeInfoCount
        gsbg = self.gsbg(resp)
        base["changeInfo"] = gsbg

        #分支机构计数  分支机构
        branchInfoCount = "".join(etre.xpath('//span[contains(text(),"分支机构")]/following-sibling::span[1]/text()'))
        base["branchInfoCount"] = None if branchInfoCount == '' else branchInfoCount
        ...
        lawDangerous["annualReportsCount"] = None if annualReportsCount == '' else annualReportsCount
        ...
        correlation["investmentInfoCount"] = None if investmentInfoCount == '' else investmentInfoCount
    ...
    def first_second_page(self,items, resp):
        ...
        items["lawDangerous"]["lawSuitsInfoCount"] = None if lawSuitsInfoCount == '' else lawSuitsInfoCount
        ...
        items["lawDangerous"]["executedPersonInfoCount"] = None if executedPersonInfoCount == '' else executedPersonInfoCount
        ...
        items["lawDangerous"]["courtNoticeInfoCount"] = None if courtNoticeInfoCount == '' else courtNoticeInfoCount
        ...
        items["lawDangerous"]["noticesInfoCount"] = None if noticesInfoCount == '' else noticesInfoCount
        ...
        items["lawDangerous"]["executionInfoCount"] = None if executionInfoCount == '' else executionInfoCount
    ...
    def oper_page(self,items, resp):
        ...
        items["lawDangerous"]["abnormalInfoCount"] = None if abnormalInfoCount == '' else abnormalInfoCount
        ...
        items["lawDangerous"]["equityInfoCount"] = None if equityInfoCount == '' else equityInfoCount
    ...
    def base_second_page(self,items,resp):
        ...
        development["financeDataInfoCount"] = None if financeDataInfoCount == '' else financeDataInfoCount
        ...
        coreteamCount = None if coreteamCount == '' else coreteamCount
    ...
    def base_three_page(self,items,resp):
        ...
        items["lawDangerous"]["development"]["admLicenseInfoCount"] = None if admLicenseInfoCount == '' else admLicenseInfoCount
        xzxk = self.xzxk(resp)
        ...
        items["lawDangerous"]["development"]["zpxxCount"] = None if zpxxinfoCount == '' else zpxxinfoCount
        ...
        items["lawDangerous"]["development"]["taxRatingCount"] = None if taxRatingCount == '' else taxRatingCount
        ...
        items["lawDangerous"]["development"]["wxnumCount"] = None if wxnumCount == '' else wxnumCount
        ...
        items["lawDangerous"]["development"]["BondInformationCount"] = None if BondInformationCount == '' else BondInformationCount
        ...
        knowledgeProperty["certificateInfoCount"] = None if certificateInfoCount == '' else certificateInfoCount
        ...
        items["base"]["productInfoCount"] = None if productInfoCount == '' else productInfoCount
        cpxx = self.cpxx(resp)
        items["base"]["productInfo"] = cpxx

        # 招投标计数 招投标
        biddingInfoCount = "".join(etre.xpath('//span[contains(text(),"招投标")]/following-sibling::span[1]/text()'))
        items["lawDangerous"]["development"]["biddingInfoCount"] = None if biddingInfoCount == '' else biddingInfoCount
    ...
    def base_four_page(self,items, resp):
        ...
        items["lawDangerous"]["knowledgeProperty"]["trademarkInfoCount"] = None if trademarkInfoCount == '' else trademarkInfoCount
        sbxx = self.sbxx(resp)
        ...
        items["lawDangerous"]["knowledgeProperty"]["patentInfoCount"] = None if patentInfoCount == '' else patentInfoCount
        zlxx = self.zlxx(resp)
        ...
        items["lawDangerous"]["knowledgeProperty"]["copyrightSoftInfoCount"] = None if copyrightSoftInfoCount == '' else copyrightSoftInfoCount
        rjzz = self.rjzz(resp)
        ...
        items["lawDangerous"]["knowledgeProperty"]["copyrightInfoCount"] = None if copyrightInfoCount == '' else copyrightInfoCount
        zpzz = self.zpzz(resp)
    ...
    def get_req(self,url):
        ...
        while count:
            # proxy = ABY()
            # resp = self.s.get(url=url,proxies=proxy,timeout=10)
...
def start(*args):
    ...
    count = 100
    while count:
        # result = red_cli.srandmember("directories")
        # key = eval(result)["company"]
        key = '国家电网有限公司'
        # _id = eval(result)["_id"]
        ...
        #     item["company"] = key
        #     item["errTag"] = "1"
        #     red_cli.sadd("err_director", str(item))
        #     red_cli.srem("directories", result)
        #     count -= 1
        # elif results == "公司名不匹配":
        #     item = {}
        #     item["_id"] = _id
